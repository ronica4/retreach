...
export default function RetreatShell({ retreat, children }: Props) {
  ...
  return (
    <div className="space-y-0">
      <div className="mb-4">
        <Link href="/dashboard" className="inline-flex items-center gap-1.5 text-sm text-stone-400 hover:text-stone-700 transition-colors mb-3">
          <ArrowLeft size={14} />
          All retreats
        </Link>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-stone-900">{retreat.name}</h1>
          ...
          <span className={cn(
            ...
            currentStage === 'planning' && 'bg-emerald-100 text-emerald-700',
            currentStage === 'active'   && 'bg-amber-100 text-amber-700',
            currentStage === 'closed'   && 'bg-stone-100 text-stone-500',
          )}>
            {currentStage}
          </span>
        </div>
